...
export async function POST(req: Request) {
  try {
    ...
    const newApp = await LoanApplication.create({
      ...
      down_payment: body.loanData?.downPayment ?? '',
      monthlyPayment: body.loanData?.monthlyPayment ?? '',
      terms: body.loanData?.loanTerm ?? '',
      ...
      empOrBusiness: body.empOrBusiness ?? null,
      designation: body.designation ?? null,
      gross_monthly_income: parseFloat(body.grossMonthlyIncome) || null,
      country: body.country,
      provinceOrState: body.provinceOrState,
      barangay: body.barrangay,
      cityOrTown: body.cityOrTown,
      streetNameAndSpecAddress: body.streetNameAndSpecAddress,
